...
def fetch_peer_review_events(assignment, name_map: Optional[dict[int, str]] = None) -> pd.DataFrame:
    """
    ...
    """
    rows = []
    for pr in assignment.get_peer_reviews():  # pr has assessor_id (reviewer) and user_id (assessed)
        reviewer_id = pr.assessor_id
        assessed_id = pr.user_id
        # Look for a comment authored by the reviewer on the assessed submission
        assessed_sub = assignment.get_submission(assessed_id, include=['submission_comments'])
        comment_time = None
        for c in getattr(assessed_sub, "submission_comments", []):
            if c.get("author_id") == reviewer_id:
                ...
                break
        rows.append(
            dict(reviewer_id=reviewer_id, assessed_user_id=assessed_id, comment_time=comment_time)
        )
...
def evaluate_peer_review_lateness(
    ...
    per["status"] = per["comment_time"].apply(
        lambda t: "missing" if pd.isna(t) else ("late" if t > pd.to_datetime(deadline_ts).tz_convert("UTC") else "on_time")
    )
    per["penalty_points"] = per["status"].isin(["late", "missing"]).astype(int) * points_per_missed_or_late
    ...
    def _comment(row):
        ...
        if row["status"] == "late":
            return (f"Your peer review for {assessed} was submitted after the deadline "
    ...
    agg = (actionable
           .groupby(["reviewer_id", "reviewer_name"], dropna=False)
           .agg(points=("penalty_points", "sum"),
                canvas_comment=("canvas_comment", lambda x: "\n+++++++++++++++++\n".join(x)))
           .reset_index())
