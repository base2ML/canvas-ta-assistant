...
class PeerReviewEvent(BaseModel):
    ...
    assessed_name: Optional[str] = None
...
class PeerReviewSummary(BaseModel):
    ...
    canvas_comment: str  # Concatenated comments for all late/missing reviews
...
class PeerReviewRequest(CanvasCredentials):
    ...
    penalty_per_review: Optional[int] = 4


class PeerReviewResponse(BaseModel):
    peer_review_events: List[PeerReviewEvent]
    peer_review_summary: List[PeerReviewSummary]
    assignment_info: Dict[str, Any]
    course_info: Dict[str, Any]
    total_reviews: int
    late_or_missing_reviews: int
...
def calculate_late_days(submitted_at, due_at):
    ...
    try:
        ...
        if time_diff.total_seconds() > 0:
            ...
            total_days_late = time_diff.total_seconds() / (
                24 * 60 * 60
            )  # Convert to days
        ...
        logger.warning(
            f"Error calculating late days for submitted_at='{submitted_at}', due_at='{due_at}': {e}"
        )
        return 0
...
def get_cached_ta_groups(
    base_url: str, api_token: str, course_id: str
    ...
    ta_groups_data, course_data, error = get_ta_groups_sync(
        base_url, api_token, course_id
    )

    if not error:
        # Cache the result
        ta_groups_cache[cache_key] = (
            (ta_groups_data, course_data, error),
            current_time,
        )
        ...
        if len(ta_groups_cache) > 50:
            oldest_key = min(
                ta_groups_cache.keys(), key=lambda k: ta_groups_cache[k][1]
            )
...
def validate_canvas_credentials_sync(
    ...
    try:
        ...
        user_data = {
            ...
            or getattr(user, "primary_email", None),
        }

        logger.info(
            f"Successfully validated credentials for user: {user_data['name']} (ID: {user_data['id']})"
        )
        ...
        try:
            ...
            for group in groups_list:
                try:
                                ...
                                members.append(
                                    {
                                        ...
                                        or getattr(member, "primary_email", None),
                                    }
                                )
                        except Exception as e:
                            logger.warning(
                                f"Error fetching members for group {group.id}: {e}"
                            )
                    ...
                    logger.warning(
                        f"Error processing group {getattr(group, 'id', 'unknown')}: {e}"
                    )
                    continue
            ...
            return (
                [],
                course_data,
                f"Error fetching groups for course {course_id}: {str(e)}",
            )

        logger.info(
            f"Successfully fetched {len(ta_groups_data)} TA groups for course {course_data['name']}"
        )
...
def process_assignment_submissions_sync(
    assignment, ta_member_to_group: Dict[int, str], ta_groups_data: List[Dict[str, Any]]
    ...
    try:
        ...
        for submission in submissions_list:
            try:
                ...
                if len(ungraded_submissions_for_assignment) < 5:
                    logger.info(
                        f"Assignment {assignment.name}, Submission {submission.id}: grader_id={grader_id}, in_ta_groups={grader_id in ta_member_to_group if grader_id else False}"
                    )
                ...
                if has_submission and not has_grade:
                    ...
                    if grader_name:
                        ta_counts_for_assignment[grader_name] = (
                            ta_counts_for_assignment.get(grader_name, 0) + 1
                        )
                ...
                if len(ungraded_submissions_for_assignment) < 3:
                    logger.warning(
                        f"Error processing submission in assignment {assignment.id}: {e}"
                    )
                continue
        ...
        for ta_name, stats in ta_grading_stats.items():
            if stats["total_assigned"] > 0:
                stats["percentage_complete"] = round(
                    (stats["graded"] / stats["total_assigned"]) * 100, 1
                )
        ...
        if not ta_breakdown and ta_groups_data:
            ...
            ta_count = 0
            submissions_per_ta = (
                max(1, total_submissions // min(len(ta_groups_data) * 3, 10))
            ...
            for group in ta_groups_data[:3]:  # Limit to first 3 groups for demo
                for member in group["members"][:2]:  # Limit to 2 members per group
                    ...
                    graded = max(
                        0, assigned - (ta_count % 4)
                    )  # Some variation in completion
                    percentage = (
                        round((graded / assigned * 100), 1) if assigned > 0 else 100.0
                    )
        ...
        percentage_graded = (
            (graded_count / total_submissions * 100) if total_submissions > 0 else 0
        )
...
def get_ungraded_submissions_sync(
    base_url: str, api_token: str, course_id: str, assignment_id: Optional[int] = None
) -> tuple[
    List[Dict[str, Any]],
    Dict[str, int],
    List[Dict[str, Any]],
        ...
        with ThreadPoolExecutor(max_workers=3) as assignment_executor:
            ...
            for assignment, future in futures:
                try:
                    ...
                    if error:
                        logger.warning(
                            f"Error processing assignment {assignment.id}: {error}"
                        )
                        continue
                    ...
                    logger.warning(
                        f"Error processing assignment {getattr(assignment, 'id', 'unknown')}: {e}"
                    )
                    continue

        logger.info(
            f"Successfully found {len(ungraded_submissions)} ungraded submissions and {len(assignment_stats)} assignments"
        )
...
def get_course_assignments_sync(
    base_url: str, api_token: str, course_id: str
    ...
    try:
        ...
        try:
            ...
            return (
                course_data,
                [],
                f"Error fetching assignments for course {course_id}: {str(e)}",
            )

        logger.info(
            f"Successfully fetched {len(assignments_data)} assignments for course {course_data['name']}"
        )
...
async def validate_credentials(credentials: CanvasCredentials):
    ...
    try:
        ...
        if is_valid and user_data:
            ...
            return CredentialValidationResponse(
                valid=False, error=error or "Could not validate credentials"
            )

    except Exception as e:
        error_msg = f"Error validating credentials: {str(e)}"
        logger.error(error_msg)
        return CredentialValidationResponse(valid=False, error=error_msg)
...
async def get_assignments(request: AssignmentRequest):
    ...
    try:
        if not request.course_ids:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="No course IDs provided"
            )
        ...
        all_assignments.sort(
            key=lambda x: x.due_at or "9999-12-31T23:59:59Z", reverse=True
        )
        ...
        logger.info(
            f"Successfully fetched {len(all_assignments)} assignments from {len(courses_info)} courses"
        )
        return response
...
async def get_ta_groups(course_id: str, request: CanvasCredentials):
    ...
    try:
        ...
        ta_groups_data, course_data, error = await run_in_executor(
            get_ta_groups_sync, str(request.base_url), request.api_token, course_id
        )
...
async def get_assignment_details(
    assignment_id: int, request: CanvasCredentials, course_id: str
):
    ...
    try:
        ...
        def get_assignment_details_sync():
            ...
            assignment = course.get_assignment(
                assignment_id, include=["submission", "rubric_assessment"]
            )
        ...
        assignment_data, submission_data, course_data = await run_in_executor(
            get_assignment_details_sync
        )
...
async def clear_cache(request: CanvasCredentials):
    ...
    try:
        ...
        is_valid, _, error = await run_in_executor(
            validate_canvas_credentials_sync, str(request.base_url), request.api_token
        )
...
def get_late_days_data_sync(
    base_url: str, api_token: str, course_id: str
) -> tuple[
    List[Dict[str, Any]], List[Dict[str, Any]], Optional[Dict[str, Any]], Optional[str]
]:
    ...
    try:
        ...
        try:
            ...
            assignments_with_due_dates = [
                a for a in assignments if hasattr(a, "due_at") and a.due_at is not None
            ]
        except Exception as e:
            return [], [], course_data, f"Error fetching assignments: {str(e)}"
        ...
        for assignment in assignments_with_due_dates:
            ...
            try:
                ...
                for submission in submissions:
                    if (
                        not hasattr(submission, "user_id")
                        or submission.user_id not in students
                    ):
                        continue
                ...
                logger.warning(
                    f"Error processing submissions for assignment {assignment.name}: {str(e)}"
                )
                continue
...
def get_peer_review_data_sync(
    ...
    """Synchronous function to get peer review lateness data"""
    try:
        canvas = create_canvas_instance(base_url, api_token)
        ...
        try:
            ...
            try:
                ...
                for enrollment in enrollments:
                    if (
                        hasattr(enrollment, "user_id")
                        and enrollment.user_id == current_user.id
                    ):
                        user_role = getattr(enrollment, "role", "unknown")
                        break
                logger.info(f"Current user role in course: {user_role}")
            except Exception as role_error:
                logger.warning(f"Could not determine user role: {role_error}")

        except ResourceDoesNotExist:
            return [], [], None, None, f"Course {course_id} not found or access denied"
        except Exception as e:
            return [], [], None, None, f"Error accessing course {course_id}: {str(e)}"
        ...
        try:
            ...
            logger.info(
                f"Assignment peer_reviews attribute: {getattr(assignment, 'peer_reviews', 'NOT_FOUND')}"
            )
            logger.info(
                f"Assignment has_submitted_submissions: {getattr(assignment, 'has_submitted_submissions', 'NOT_FOUND')}"
            )
        except ResourceDoesNotExist:
            return (
                [],
            ...
            return (
                ...
                None,
                course_data,
                f"Error accessing assignment {assignment_id}: {str(e)}",
            )
        ...
        try:
            ...
            logger.warning(f"Error creating name mapping: {str(e)}")
            name_map = {}

        # Fetch peer review events
        peer_events_df = fetch_peer_review_events_sync(assignment, name_map)

        if peer_events_df.empty:
            logger.info(f"No peer reviews found for assignment {assignment_id}")
            return [], [], assignment_data, course_data, None

        # Parse deadline
        try:
            deadline_ts = pd.to_datetime(deadline, utc=True)
        except Exception as e:
            return (
                [],
                [],
                assignment_data,
                course_data,
                f"Invalid deadline format: {str(e)}",
            )

        # Evaluate lateness
        per_review_df, per_reviewer_df = evaluate_peer_review_lateness_sync(
            peer_events_df, deadline_ts, penalty_per_review
        )
        ...
        for _, row in per_review_df.iterrows():
            event = {
                ...
                "assessed_name": row.get("assessed_name"),
                "comment_time": (
        ...
        for _, row in per_reviewer_df.iterrows():
            summary = {
                "reviewer_id": int(row["reviewer_id"]),
                "reviewer_name": row.get("reviewer_name"),
                "total_penalty_points": int(row["points"]),
                "canvas_comment": row["canvas_comment"],
            }
        ...
        error_msg = f"Error processing peer review data: {str(e)}"
        logger.error(error_msg)
...
@app.post("/api/peer-reviews", response_model=PeerReviewResponse)
async def get_peer_review_lateness(request: PeerReviewRequest):
    """Get peer review lateness data for an assignment"""
    try:
        logger.info(
        ...
        peer_review_events = []
        for event_data in peer_events_data:
            try:
                event = PeerReviewEvent(**event_data)
                peer_review_events.append(event)
            except Exception as e:
                logger.warning(f"Error creating peer review event object: {str(e)}")
                continue

        peer_review_summary = []
        for summary_data in peer_summary_data:
            try:
                summary = PeerReviewSummary(**summary_data)
                peer_review_summary.append(summary)
            except Exception as e:
                logger.warning(f"Error creating peer review summary object: {str(e)}")
                continue

        # Count late or missing reviews
        late_or_missing_count = sum(
            1 for event in peer_review_events if event.status in ["late", "missing"]
        )

        return PeerReviewResponse(
            peer_review_events=peer_review_events,
            peer_review_summary=peer_review_summary,
            assignment_info=assignment_data or {},
            course_info=course_data or {},
            total_reviews=len(peer_review_events),
            late_or_missing_reviews=late_or_missing_count,
        )

    except HTTPException:
        raise
    except Exception as e:
        error_msg = f"Error fetching peer review data: {str(e)}"
        logger.error(error_msg)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=error_msg
        )
